...
test_data = {
    "tipo": "Helicoidal",          # "Reto" ou "Helicoidal"
    "direcao": "Direita",

    "diametro primitivo": 120,       # 0 -> calcula automaticamente
    "k dentes": 0,                 # 0 -> calcula automaticamente

    "angulo de helice": 15,        # graus
    "angulo de pressao": 20,       # graus

    "modulo": 3.5,
    "numero de dentes": 32,
    "largura do dentado": 5,
}
...
class Roda:
    ...
    def correcao(self):
        ...
        # com correção
            ...
            self.alpha_ap_func = acos(
                ((self.entre_eixo / 2) / self.d1)
                * cos(self.alpha_ap)
            )

            self.inv_alpha_ap_func = (
                tan(self.alpha_ap_func)
                - self.alpha_ap_func
            )
            ...
            self.Bv = (
                cos(self.alpha_ap)
                / cos(self.alpha_ap_func)
                - 1
            )

            self.K = self.z1 * (self.B - self.Bv)

            self.d_a = self.modulo * (
                self.z1 / cos(self.beta)
                + 2
                + 2 * self.x1
                - 2 * self.K
            )
    ...
    def cota_sobre_k_dentes(self):
        ...
        if self.k_dentes == 0:
            ...
            self.k_dentes = round(
                ...
                * (
                    tan(self.alpha_vt)
                    / (cos(self.beta_base) ** 2)
                    - self.inv_alpha_ap
                    - 2 * self.x1 * tan(self.alpha)/self.z1
                )
                + 1
            )
...
roda.correcao()
roda.backlash()
roda.cota_sobre_k_dentes()

print("===================================")
print("RESULTADOS")
print("===================================")

print(f"d1 = {roda.d1:.3f}")
print(f"x1 = {roda.x1:.6f}")
print(f"d_a = {roda.d_a:.3f}")

print()

print(f"Ess = {roda.Ess}")
print(f"Esi = {roda.Esi}")
print(f"Ts = {roda.Ts}")

print()

print(f"jt = {roda.jt:.3f}")
print(f"jn = {roda.jn:.3f}")

print()

print(f"k dentes = {roda.k_dentes}")

print()

print(f"Wk = {roda.Wk:.3f}")
print(f"Tw = {roda.Tw:.3f}")

print()

print(f"Wk final = {roda.Wk_final}")